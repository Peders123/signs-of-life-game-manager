...
def main():
    ...
    if os.path.exists("sheets/token.json"):
        credentials = Credentials.from_authorized_user_file("sheets/token.json", SCOPES)
    ...
    try:
        ...
        result = sheets.values().get(spreadsheetId=SPREADSHEET_ID, range="Sheet1!A1").execute()
